...
$dataarr['user_category']=$this->input->post('category');
 if(isset($_FILES['profile_image']['name']) && !empty($_FILES['profile_image']['name'])){
	 ...
	 $profile_image = fileUploading('profile_image','images','gif|jpg|png|jpeg|JPG');
	...
	$dataarr['profile']   = base_url().$profile_image_thumb;
			} 
 }
...
$info=array('tb_users.id','tb_users.firstname','tb_users.lastname','tb_users.email','tb_users.profile','tb_users.city','tb_users.state','tb_users.country','tb_users.user_role','tb_users.business_name');
            ...
            $condition=array('requests.sender'=>$cuid,'requests.status'=>0,'job_requested_by!='=>get_current_user_id(0));
...
foreach($user_data['result'] as $dat ){
	...
	$datas['comments']=$commentData['result'];
	$datas['status']=0;
	$mdata[]=$datas;
}
}
...
$info=array('tb_users.id','tb_users.firstname','tb_users.lastname','tb_users.email','tb_users.profile','tb_users.city','tb_users.state','tb_users.country','tb_users.user_role','tb_users.business_name');
    ...
    if($user_data){
		$resp=array("result"=>1,'msg'=>'User Details .','data'=>array('profiledata'=>$user_data,'posts'=>$user_posts['result'],'rating'=>$user_rating,'user_reviewCount'=>$user_reviewCount,'qr_image'=>$myqr_image,'favourite'=>$favourite,'album'=>$user_album,'friends'=>$data,'performance'=>$performance));
		$this->response($resp, REST_Controller::HTTP_OK);
		}else{
		$resp=array("result"=>0,"msg"=>"Somethig went wrong, Try again.",'data'=>0);
		$this->response($resp, REST_Controller::HTTP_OK);
...
if(!empty($result)){
	...
	$resp=array("result"=>1,'data'=>array('userdata'=>$user_data),'msg'=>'Successfully Login.');
	$this->response($resp, REST_Controller::HTTP_OK);	
...
public function getallreviews_post(){
        ...
        if($cuid){
            $data['anonymous'] = $this->common_model->getsingle(RATINGS,array('rated_by_user'=>$cuid,'rated_to_user'=>$user_id));
        }
		//pr($data);
...
if(!empty($this->input->post('current_position'))){
$pro_skills=implode(',',$this->input->post('current_position'));
$dataArr['professional_skill']=$pro_skills;
}
if(!empty($this->input->post('additional_services'))){
$add_skills=implode(',',$this->input->post('additional_services'));
...
$end=5;	
 ...
 $contactIDs = array();
        ...
        if(!empty($contactIDs)){
            ...
            $data['highlights'] = $this->common_model->GetJoinRecord(USERS,'id',POSTS,'user_id','tb_users.firstname,tb_users.lastname,tb_users.business_name,tb_users.user_role,tb_users.profile,tb_users.id as user_id1,posts.*',$condition,'','posts.id','DESC',$end,$start);
        }
		//lq();
		//QR code generate
        $data['img_url']="";
        $this->load->library('ciqrcode');
        $qr_image=rand().'.png';
        $params['data'] = base_url().'viewdetails/profile/'.encoding($cuid)."?review=1";
        $params['level'] = 'H';
        $params['size'] = 5;
        $params['savename'] =FCPATH."qr_code/".$qr_image;
        if($this->ciqrcode->generate($params))
...
public function mytask_post(){
		...
		$data = $this->common_model->GetJoinRecord(TASK,'id',TASK_ASSIGNED,'task_id','task.*,task_assigned.note',array('task_assigned.user_id'=>$user_id),'','created_date','DESC');
...
if(isset($_FILES['group_icon']['name']) && !empty($_FILES['group_icon']['name'])){
	...
	$dataInsert['group_icon']   = base_url().'uploads/users/'.$profile_pic;
		...
		$resp=array("result"=>0,"msg"=>"Please select group icon to upload.",'data'=>array());
		$this->response($resp, REST_Controller::HTTP_OK); 
 }
 /****************/
...
$grouplist = $this->common_model->getAllwhere(MESSAGE_GROUP,$condition, 'id', 'DESC','all');
    ...
    if($grouplist){
		$resp=array("result"=>1,'msg'=>'Group List','lists'=>$grouplist);
		$this->response($resp, REST_Controller::HTTP_OK);
		}else{
		$resp=array("result"=>0,"msg"=>"Somethig went wrong, Try again.",'lists'=>"");
		$this->response($resp, REST_Controller::HTTP_OK);
