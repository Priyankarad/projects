...
// define('OAUTH_CONSUMER_KEY', 'dj0yJmk9TWE5UXNNaGc3MU0xJmQ9WVdrOWF6Sm5kMnM0TjJVbWNHbzlNQS0tJnM9Y29uc3VtZXJzZWNyZXQmeD01Zg--');
// define('OAUTH_CONSUMER_SECRET', 'f13e500773f45817ca4bbd83529d74cab68987e6');

define('OAUTH_CONSUMER_KEY', 'dj0yJmk9d3N5eW9QalNCQ05GJnM9Y29uc3VtZXJzZWNyZXQmc3Y9MCZ4PWY5');
define('OAUTH_CONSUMER_SECRET', 'dc1d22cfd36a0f2a43cfee646a1764dba4596911');
...
//print(json_pretty_print($body));
        ...
        $newList = array();
// echo '<pre>';
// print_r($yahoo_array) ;die;
        foreach($yahoo_array as $key=>$values){
            $count = 0;
            foreach($values->contact as $keys=>$values_sub){
//$giverName = isset($values_sub->fields[0]->value->givenName;
                // $email= $values_sub->fields[1]->value;

                // if(is_string($email) && trim($email)!=""){
                //     $newList[$count]['name']   = isset($values_sub->fields[0]->value->givenName)?$values_sub->fields[0]->value->givenName:'';
                //     $newList[$count]['email']   = $email;
                //     $newList[$count]['image'] = base_url().'assets/images/default_mail.jpg';
                //     $count++;
                // }
                if(!empty($values_sub->fields)){
                    //for name & email
                    foreach($values_sub->fields as $row){
                        if(isset($row->type) && $row->type == 'name'){
                            $name   = ucfirst($row->value->givenName);
                            if(isset($row->value->familyName)){
                                $name.= " ".ucfirst($row->value->familyName);
                            }
                            $newList[$count]['name'] = $name;
                        }
                        if(isset($row->type) && $row->type == 'email'){
                            $newList[$count]['email']   = $row->value;
                        }
                    }
                    $newList[$count]['image'] = base_url().'assets/images/default_mail.jpg';
                    $count++;
