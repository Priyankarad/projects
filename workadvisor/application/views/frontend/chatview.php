...
function saveFirebaseMessage(){
	...
	if(message!='' || type!=''){
		...
		$.ajax({
			type:'POST',
			url:site_url+'user/firebaseMessageNotification',
			data: {sender:sender,receiver:receiver,name_sender:senderName,groupSet:groupSet},
			dataType: 'json',
...
reference.ref('messages/'+chat).on('value', function(snapshot) {
	...
	if(msgData!=null){
		...
		for(i=0;i<key.length;i++){
			...
			var senderRow = msgData[index]['sender'];
			if((receiverRow!='undefined') && (senderRow!='undefined')){
				if(is_group!=''){
					...
					if(($.inArray(sender,readBy) === -1) && $.isNumeric(receiverRow) && (receiverRow == sender) &&  ((receiverRow == sender) || (receiverRow == receiver)) &&  ((senderRow == sender) || (senderRow == receiver)) ){
						readBy.push(sender);
						reference.ref().child('messages/'+chat+'/'+index).update({ read_by: readBy});
					}
				}
