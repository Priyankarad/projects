...
<script> 
    ...
    function saveFirebaseMessage(){
        ...
        reference.ref('recents/'+chat+'/').once('value', function(snapshot) {
            ...
            senderCount = recentData['unread_'+sender];
            var recentCount = {};
            recentCount['unread_'+receiver] = receiverCount;
            ...
            recentArr['time'] = messageDate;
            recentArr['type'] = '';
            recentArr['typing'] = 0;
            recentArr['unread_'+sender] = senderCount;
            recentArr['unread_'+receiver] = receiverCount;
            recentArr['typing'] = 0;
            recentArr['video'] = video;
            reference.ref().child('recents/'+chat+'/').update(recentArr);
...
$(document).on('keyup','.tetx_bx',function(){
    ...
    reference.ref().child('recents/'+chat+'/').update({typing:0});
});
...
$(document).on('keydown','.tetx_bx',function(){
    ...
    reference.ref().child('recents/'+chat+'/').update({typing:1});
});
</script>