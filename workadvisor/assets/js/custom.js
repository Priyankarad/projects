...
$('#review_filter').trigger('change');
    ...
    $('#searchName').keyup(function(){
        ...
        $.ajax({
            ...
            data: {search:search},
            success: function (data) {
                $('#friendsList').html(data.html);
                if(data.group!=''){
                    getIndivisualMsg(data.friendID,data.isGroup);
                }
    ...
    $(document).on('click','.likePost',function(){
        var status = $(this).find('.likeSpan').html();
        var likeCount = $(this).find('.likeCount').html();
        if(status == 'Like'){
            $(this).find('.likeSpan').html('Liked');
            likeCount++;
            $(this).find('.likeCount').html(likeCount);
        }else{
            $(this).find('.likeSpan').html('Like');
            likeCount--;
            if(Number(likeCount)>0){
                $(this).find('.likeCount').html(likeCount);
            }else{
                $(this).find('.likeCount').html('');
            }
        }
        
        var postID = $(this).closest('.post-id').attr('id');
        var otherUser = $(this).closest('.post-id').data('uid');
        $.ajax({
            url: site_url+'user/likePost',
            type: "post",
            dataType: 'json',
            data:{otherUser:otherUser,postID:postID},
            success: function(result){
                if(result.status=='0' && result.msg=='Login first'){
                    $('#LoginModal').modal('show');
                }
            }
        });
...
/*To change task modal text*/
$(document).on('click','.taskAdd',function(){
    $('#taskForm')[0].reset();
    $('#task_members').select2({
        placeholder: 'Enter Users'
    });
    $('.taskEdit').text('Task Add');
    $('.taskAddBtn').text('Add Task');
    $('.delete_task').hide();
});

/*To edit task*/
$(document).on('click','.taskEdits',function(){
    $('#taskForm')[0].reset();
    $('.taskEdit').text('Task Edit');
    $('.taskAddBtn').text('Update Task');
    var taskID = $(this).data('task_id');
    $.ajax({
        type: "POST",
        dataType: "json",
        url: site_url+'user/taskData',
        data: {taskID:taskID},
        success: function (data) {
            if(data.taskDataArr!=''){
                var taskID = data.taskDataArr[0].id;
                var title = data.taskDataArr[0].title;
                var description = data.taskDataArr[0].description;
                var startDate = data.taskDataArr[0].start_date;
                var endDate = data.taskDataArr[0].end_date;
                var users = data.taskDataArr[0].user_ids;
                var userArr = users.split(',');
                $('#title').val(title);
                $('#description').val(description);
                $('#start_dates').val(startDate);
                $('#end_dates').val(endDate);
                $("#task_members").select2().val(userArr).trigger("change");
                $('#task_id').val(taskID);
                setID(taskID,'task');
                $('.delete_task').show();
            }
...
/*To fetch my task list*/
function getMyTasks(){
    $.ajax({
        url : site_url+'user/getMyTasks',
        type: 'post',
        data : {id:0},
        dataType : 'json',
        success: function (data) {
            $('#my_tasks').html(data.html);
            $('.task_bell').text(data.taskCount);
        }
    });
}

/*To fetch task info*/
$(document).on('click','.myTaskList',function(){
    var taskID = $(this).data('mytask_id');
    var assignedTo = $(this).data('assigned_to');
    var notification = $(this).data('notification');//for notification
    $.ajax({
        url : site_url+'user/getMyTasks',
        type: 'post',
        data : {taskID:taskID,assignedTo:assignedTo,notification,notification:notification},
        dataType : 'json',
        success: function (data) {
            var title = data.taskData.title;
            var description = data.taskData.description;
            var startDate = data.taskData.start_date;
            var endDate = data.taskData.end_date;
            var taskId = data.taskData.id;
            var note = data.taskData.note;
            var status = data.taskData.status;
            $('#task_title').val(title);
            $('#task_description').text(description);
            $('#task_start').val(startDate);
            $('#task_end').val(endDate);
            $('#taskId').val(taskId);
            $('#note').text(note);
            $('#status').val(status);
            if(assignedTo != 0){
