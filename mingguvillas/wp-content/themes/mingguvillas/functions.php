...
function create_post_type() {
   ...
   register_post_type( 'kerobokan',
    array(
      'labels' => array(
        'name' => __( 'Kerobokan' ),
        'singular_name' => __( 'kerobokan' )
      ),
      'public' => true,
      'has_archive' => true,
      'supports' => array( 'title', 'editor', 'comments', 'trackbacks', 'author', 'excerpt', 'custom-fields', 'thumbnail' ),
       //'taxonomies' => array('category'),
      'rewrite' => array( 'slug' => 'kerobokan', 'with_front' => false ),
    )
  );
...
function create_post_type_one() {

   register_post_type( 'seminyak',
    array(
      'labels' => array(
        'name' => __( 'Seminyak' ),
        'singular_name' => __( 'seminyak' )
      ),
      'public' => true,
      'has_archive' => true,
      'supports' => array( 'title', 'editor', 'comments', 'trackbacks', 'author', 'excerpt', 'custom-fields', 'thumbnail' ),
       //'taxonomies' => array('category'),
      'rewrite' => array( 'slug' => 'seminyak', 'with_front' => false ),
    )
  );

}
add_action( 'init', 'create_post_type_one' );


function extsite_post($url,$data) 
{
...
function getAvailability($propertyID,$from,$to){
	...
	$result =  extsite_post($url,$xml_data); 
	return $result;
}
...
function create_post_type_category() {
   ...
   register_post_type( 'Review',
    ...
    register_post_type( 'FoodGallery',
    array(
      'labels' => array(
        'name' => __( 'FoodGallery' ),
        'singular_name' => __( 'FoodGallery' )
      ),
      'public' => true,
      'has_archive' => true,
      'supports' => array( 'title', 'editor', 'comments', 'trackbacks', 'author', 'excerpt', 'custom-fields', 'thumbnail' ),
       //'taxonomies' => array('category'),
      'rewrite' => array( 'slug' => 'FoodGallery', 'with_front' => false ),
    )
  );
...
function why_us_custom_taxonomy() {
  ...
  register_taxonomy('categorye',array('foodgallery'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
  ...
  register_taxonomy('category',array('review'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
    'show_admin_column' => true,
  ...
  register_taxonomy('categories',array('photogallery'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
    'show_admin_column' => true,
...
add_action( 'init', 'why_us_custom_taxonomy', 0 );
...
//   $conv_id = "{$from}_{$to}";
//   $string = file_get_contents("https://free.currencyconverterapi.com/api/v6/convert?q=$conv_id&compact=ultra&apiKey=f739e98d9aa9858daf29");
//   $json_a = json_decode($string, true);

//   return $amount * round($json_a[$conv_id], 4);
// }
add_action('init','getLiveCurrencyRates',0);
	...
	if(empty($_SESSION['exchangeRates'])){
		...
		if(false !== $response_json) {
			try {
				...
				if(!empty($response_object->rates)){
					$_SESSION['exchangeRates'] = $response_object->rates;
				}
			}
			catch(Exception $e) {
