...
if(isset($_REQUEST['doresetpass']) && !empty($_REQUEST['doresetpass']) && $_REQUEST['doresetpass'] == 'yes'){
	...
	if($flag == 1){
		...
		header("Location:".BASE_URL.$_SESSION['currentLang'].'/resetpassword/merchant/'.$identifier.'?action=success');
		exit;
	}
	...
	<div class="container">
		...
		<section class="investorsignup">
			...
			<div id="message-success">
			 <i class="icon-ok"></i><?php echo($transArr['Your password has been successfully reset. Please']); ?> <a href="<?=BASE_URL.$getLang?>/merchant-signin"><?php echo($transArr['Click here to Sign In']); ?></a><br />
			</div>
			<?php
			}
			?>
