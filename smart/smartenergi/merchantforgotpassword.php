...
if(isset($_REQUEST['doresetpass']) && !empty($_REQUEST['doresetpass']) && $_REQUEST['doresetpass'] == 'yes'){
	...
	if(empty($email)){
		...
		$checkemailRow = selectQuery("backoffice_merchants","email = '".$email."'",$con);
	...
	if($flag == 1){
		...
		$updateSql = updateQuery("backoffice_merchants",
									array("id"=>$lastid),
									array("unique_identifier"=>addslashes($uniqueid)),
									$con);					  		
		
		// Send Reset Password Email
		
		$request = curl_init(BASE_URL.'backoffice/merchantforgotpassword/'.$lastid);
		curl_setopt($request, CURLOPT_RETURNTRANSFER, true);
		$response = curl_exec($request);
		curl_close($request);
		
		header("Location:".BASE_URL.$_SESSION['currentLang'].'/merchant-forgot-password?action=sent');
		exit;
	}
