...
if($_SERVER['REMOTE_ADDR'] == '127.0.0.1' || $_SERVER['REMOTE_ADDR'] == '::1')
{
	...
	define("DB_USER","root");
	define("DB_PASS","");
	define("DB_NAME","smartcredit");
	define("BASE_URL","http://localhost/smartcredit/");
	define("ADMIN_URL","http://localhost/smartcredit/smccontrolpanel/");
	define("ADMIN_DIR","smccontrolpanel/");
	define("HTTP_HOST",'http://');
}
else
{
	define("DB_HOST","db798353820.hosting-data.io");
	define("DB_USER","dbo798353820");
	define("DB_PASS","DDxz4W?$");
	define("DB_NAME","db798353820");
	define("BASE_URL","http://www.smartenergi.es/");
	define("ADMIN_URL","http://www.smartenergi.es/smccontrolpanel/");
	define("ADMIN_DIR","smccontrolpanel/");
	define("HTTP_HOST",'http://');
	
	/* define("DB_HOST","localhost");
	define("DB_USER","db_user_subh");
	define("DB_PASS","xh[-[)v;T3aB");
	define("DB_NAME","db_subhasis005");
	define("BASE_URL","http://subhasislaha.com/projects/smartcredit/");
	define("ADMIN_URL","http://subhasislaha.com/projects/smartcredit/smccontrolpanel/");
	define("ADMIN_DIR","smccontrolpanel/");
	define("HTTP_HOST",'http://'); */
}
...
while($rowconfig = mysqli_fetch_array($getconfig))
{
	...
	define(strtoupper($rowconfig['config_type']),stripslashes($rowconfig['config_val']));
}
...
define("HOME_BANNER",$config['home_banner']);
...
function getVariables($lang,$con){	
	
	$variables = array();
	
	// Get Status
	
	$getdata = "SELECT DVT.value, DV.id FROM ".TABLE_PREFIX."dropdown_values AS DV 
				LEFT JOIN ".TABLE_PREFIX."dropdown_values_translations AS DVT ON DVT.value_id = DV.id
				WHERE DV.flag = '1' AND DV.type = 'living_status' AND DVT.language_id = '".$lang."'";
	...
	if(count($numdata)){
		...
		while($rowdata = mysqli_fetch_assoc($getdata)){
			...
			$status[$rowdata['id']] = $rowdata['value'];
		}
	}
	...
	$getdata = "SELECT DVT.value, DV.id FROM ".TABLE_PREFIX."dropdown_values AS DV 
				LEFT JOIN ".TABLE_PREFIX."dropdown_values_translations AS DVT ON DVT.value_id = DV.id
				WHERE DV.flag = '1' AND DV.type = 'marital_status' AND DVT.language_id = '".$lang."'";
	...
	if(count($numdata)){
		...
		while($rowdata = mysqli_fetch_assoc($getdata)){
			...
			$marital_status[$rowdata['id']] = $rowdata['value'];
		}
	}
	...
	$getdata = "SELECT DVT.value, DV.id FROM ".TABLE_PREFIX."dropdown_values AS DV 
				LEFT JOIN ".TABLE_PREFIX."dropdown_values_translations AS DVT ON DVT.value_id = DV.id
				WHERE DV.flag = '1' AND DV.type = 'employment_type' AND DVT.language_id = '".$lang."'";
	...
	if(count($numdata)){
		...
		while($rowdata = mysqli_fetch_assoc($getdata)){
			...
			$employment_type[$rowdata['id']] = $rowdata['value'];
		}
	}
	...
	$getdata = "SELECT DVT.value, DV.id FROM ".TABLE_PREFIX."dropdown_values AS DV 
				LEFT JOIN ".TABLE_PREFIX."dropdown_values_translations AS DVT ON DVT.value_id = DV.id
				WHERE DV.flag = '1' AND DV.type = 'service_type' AND DVT.language_id = '".$lang."'";
	...
	if(count($numdata)){
		...
		while($rowdata = mysqli_fetch_assoc($getdata)){
			...
			$service_type[$rowdata['id']] = $rowdata['value'];
		}
	}
	...
	$getdata = "SELECT DVT.value, DV.id FROM ".TABLE_PREFIX."dropdown_values AS DV 
				LEFT JOIN ".TABLE_PREFIX."dropdown_values_translations AS DVT ON DVT.value_id = DV.id
				WHERE DV.flag = '1' AND DV.type = 'security_questions' AND DVT.language_id = '".$lang."'";
	...
	if(count($numdata)){
		...
		while($rowdata = mysqli_fetch_assoc($getdata)){
			...
			$security_questions[$rowdata['id']] = $rowdata['value'];
		}
	}
	...
	$getdata = "SELECT DVT.value, DV.id FROM ".TABLE_PREFIX."dropdown_values AS DV 
				LEFT JOIN ".TABLE_PREFIX."dropdown_values_translations AS DVT ON DVT.value_id = DV.id
				WHERE DV.flag = '1' AND DV.type = 'merchant_prod_type' AND DVT.language_id = '".$lang."'";
	...
	if(count($numdata)){
		...
		while($rowdata = mysqli_fetch_assoc($getdata)){
			...
			$merchant_prod_type[$rowdata['id']] = $rowdata['value'];
		}
	}

	// Get Merchant Product Types
	
	$getdata = "SELECT BL.bank_name, BL.id FROM ".TABLE_PREFIX."bank_list AS BL";
	...
	if(count($numdata)){
		...
		while($rowdata = mysqli_fetch_assoc($getdata)){
			...
			$bank_list[$rowdata['id']] = $rowdata['bank_name'];
		}
	}

	
	$variables['bank_list'] = $bank_list;
...
/**** Pixlrit *****/
    function mediaImage($image_title,$con){
    	$getdata = "SELECT * FROM ".TABLE_PREFIX."media_library 
				WHERE image_title ='".$image_title."'";
