...
{
	mysqli_query($con,"UPDATE ".TABLE_PREFIX."languages SET is_default = '0'") or die(mysqli_error());
	
	$update = "UPDATE ".TABLE_PREFIX."languages SET
			   language = '".addslashes($_POST['language'])."',
			   code = '".addslashes($_POST['code'])."',
			   is_default = '".addslashes($_POST['is_default'])."',
...
if(isset($_POST['mode']) && $_POST['mode'] == "delsel")
{
	...
	{
		$del = "DELETE FROM ".TABLE_PREFIX."languages WHERE id = '".$valids."'";
...
{
	$del = "DELETE FROM ".TABLE_PREFIX."languages WHERE id = '".$id."'";
	mysql_query($del) or die(mysql_error());
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> <i class="glyphicon glyphicon-flag"></i> <?=$title?> </h1>
			  ...
			  <div class="box box-success">
				...
				<div class="box-body table-responsive">
					...
					<tbody>
					  ...
					  <?php
						$sl = 1;
						$getrows = "SELECT * FROM ".TABLE_PREFIX."languages ORDER BY id DESC";
						...
						while($row = mysqli_fetch_array($getrows)){
							...
							<tr >
								...
								<td class="center"><?=stripslashes($row['language'])?></td>
								<td class="center"><?=stripslashes($row['code'])?></td>
								<td class="center"><?=$row['is_default'] == '0' ? 'No' : 'Yes'?></td>
			  ...
			  <div class="box box-success">
				...
				<div class="box-body">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" placeholder="Language" name="language" id="language" value="<?php echo stripslashes($rowdet['language']); ?>" required />
					</div>
					
					<div class="form-group">
						<label>Code</label>
						<input type="text" class="form-control" placeholder="Code" name="code" id="code" value="<?php echo stripslashes($rowdet['code']); ?>" required />
					</div>
					...
					if($mode == 'edit'){
						...
						<div class="form-group">
							...
							<input type="radio" class="form-control" name="is_default" value="1" required <?php echo($rowdet['is_default'] == '1' ? 'checked' : ''); ?> required/>&nbsp;Yes&nbsp;&nbsp;						
							<input type="radio" class="form-control" name="is_default" value="0" required <?php echo($rowdet['is_default'] == '0' ? 'checked' : ''); ?> required/>&nbsp;No&nbsp;&nbsp;	
