		...
		if($request->has('act')){
			...
			if($act == 'doadd'){
				...
				if($flag == 1){
					...
					$borrowerWalletid = uniqid("PHP-Borrower-");
					...
					if(isset($response->RegisterWalletResult->E)){
							if($response->RegisterWalletResult->E->Code==204){
			                    ...
			                    if(!empty($GetWalletbyemailDetails->GetWalletDetailsResult->WALLET->ID)){
			                    	$borrowerWalletid=$GetWalletbyemailDetails->GetWalletDetailsResult->WALLET->ID;
						...
						$borrowerWalletid=	$response->RegisterWalletResult->WALLET->ID;
						$wallet_balance=0.00;
						$checkStatus=Lemonway::UpdateWalletStatus($borrowerWalletid,6);
