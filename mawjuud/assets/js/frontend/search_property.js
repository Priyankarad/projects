...
$.ajax({
    ...
    success: function (data) {
        ...
        /*for header tab nearby sale rent*/
        rentSale = $('#rentSale').val();
        if((rentSale!='undefined') && (rentSale!=0)){
            $('#listing').val(rentSale);
            if(rentSale == 'sale'){
                $('.sale_class a').addClass('active-menu');
            }else if(rentSale == 'rent'){
                $('.rent_class a').addClass('active-menu');
            }
            rentSale = 'undefined';
            saleRent = 1;
...
var rentSale1 = $('#rentSale1').val();
if(rentSale1!='' && rentSale1!==undefined){
    $('#listing').val(rentSale1);
}
var baths1 = $('#baths1').val();
if(baths1!='' && baths1!==undefined){
    $('#baths').val(baths1);
}
var beds1 = $('#beds1').val();
if(beds1!='' && beds1!==undefined){
    $('#beds').val(beds1);
}
// var min1 = $('#min1').val();
// var max1 = $('#max1').val();
// if((min1!='' && min1!==undefined) && (max1!='' && max1!==undefined)){
//     setPriceRangeK(min1,max1);
// }else if(min1!='' && min1!==undefined){
//     setPriceRangeK(min1,'');
// }else if(max1!='' && max1!==undefined){
//     setPriceRangeK('',max1);
// }
...
$(document).ready(function(){
    ...
    $('body').on('click', 'ul#minpriceappend li a', function(){
        ...
        minP = minPrice.replace('AED ','');
        setPriceRangeK(minP,maxP);
        priceChange();
    });
    $('body').on('click', '#maxpriceappend li a', function(){
        ...
        var maxPrice = maxPrice.replace(',','');
        maxP = maxPrice.replace('AED ','');
        setPriceRangeK(minP,maxP);
        searchProperty();
    });
    ...
    /*For converting min and max price to 'K' on field's keyup*/
    ...
    function setPriceRangeK(minPrice,maxPrice){
        var priceRange='';
        if((minPrice!==undefined && minPrice!=='') && (maxPrice===undefined || maxPrice==='')){
            priceRange+=kFormatter(minPrice)+'+';
        }
        else if((minPrice===undefined || minPrice==='') && (maxPrice!==undefined && maxPrice!=='')){
            if(maxPrice!='Any Price')
                priceRange+=('Up To '+kFormatter(maxPrice));
            else
                priceRange+='Any Price';
        }else if(minPrice!==undefined && minPrice!=='' && maxPrice!==undefined && maxPrice!==''){
            priceRange+=(kFormatter(minPrice)+'-'+kFormatter(maxPrice));
        }else{
            priceRange = 'Any Price';
        }
        $('.appendPrice').html(priceRange);
    }
    ...
    $('.titleSubmit').click(function(){
        ...
        $('#titleModal').modal('close');
        $('#title').val('');
        $.ajax({
            url: baseUrl+ 'property/savePropertySearch',
            type: 'post',
            dataType: 'json',
            data: {area:area,price:price,beds:beds,category:category,baths:baths,min_sqft:min_sqft,max_sqft:max_sqft,min_price:min_price,max_price:max_price,days_zillow:days_zillow,keywords:keywords,sort:sort,type:type,listing:listing,title:title,rentSale:saleRent},
            success: function (data) {
...
function searchProperty(poly=false){
    ...
    if(type != 'table'){
        ...
        var tableHeader = '<div class="row"><div class="col-md-12"><div class="mybgcover"></div><table class="table table-bordered responsive-table searchshortingTbM" id="tableView"><thead><th><div></div></th><th><div>Type</div></th><th><div>Title</div></th><th><div>Address</div></th><th id="Mshortprc"><div>Price(AED) <img src="assets/images/map-icon/shorting-icon.png" alt="images" class="miniconM"/></div></th><th id="Mshortbed"><div>Bed(s) <img src="assets/images/map-icon/shorting-icon.png" alt="images" class="miniconM"/></div></th><th id="Mshortbath"><div>Bath(s) <img src="assets/images/map-icon/shorting-icon.png" alt="images" class="miniconM"/></div></th><th id="Mshortsqft"><div>Size (Sq. ft.) <img src="assets/images/map-icon/shorting-icon.png" alt="images" class="miniconM"/></div></th><th><div>Favorite</div></th><th><div>Compare</div></th><th><div>Hide</div></th></thead></table></div></div>';
        ...
        if(schoolSet == 1  && (schoolsP)){
            for (i = 0; i < schoolsP.length; i++) {
                marker = new google.maps.Marker({
                    ...
                    map: map,
                    icon: baseUrl+'assets/images/school-newicon.png'
                });
                google.maps.event.addListener(marker, 'mouseover', (function(marker, i) {
                    return function() {
        ...
        if((transportSet == 1) && (transportsP)){
            for (i = 0; i < transportsP.length; i++) {
                marker = new google.maps.Marker({
                    ...
                    map: map,
                    icon: baseUrl+'assets/images/transport-newicon.png'
                });
                google.maps.event.addListener(marker, 'mouseover', (function(marker, i) {
                    return function() {
...
$(document).on('click','.M-maphide', function(){
    ...
    photoView = 0;
    $('.inner-footers').addClass('innerftbg');
});
...
$(document).on('click','.M-mapless', function(){
    ...
    photoView = 0;
    $('.inner-footers').removeClass('innerftbg');
})
...
function drawFreeHand(){
    $('.drawActive').addClass('activebg');
    $('.nearbyActive').removeClass('activebg');
    $('.clearActive').removeClass('activebg');
    latlng = '';
    propertyID = [];
    schoolID = [];
    transportID = [];
...
$(document).on('click','.nearby',function(){
    ...
    nearbySet = 1;
    drawSet = 0;
    $('.drawActive').removeClass('activebg');
    $('.clearActive').removeClass('activebg');
    $('.nearbyActive').addClass('activebg');
    /*For removing the old current location*/
    for (var i = 0; i < nearbyMarkers.length; i++) {
        nearbyMarkers[i].setMap(null);
    }
...
function nearby(lat,lng,_this=false){
    transportID = [];
    schoolID = [];
    var radius = 10000;
    for(var i=0; i<times; i++){
...
function facilities(_this,img,typeF1){
    ...
    if(drawSet == 1 || nearbySet == 1){
        if((typeF1 == 'transport') && (transportID.length>0)){
            $status = 1;
        }else if((typeF1 == 'school') && (schoolID.length>0)){
            $status = 1;
        }
    }else {
        $status = 1;
    ...
    if($status == 1){
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {typeF:typeF1,schoolID:schoolID,transportID:transportID,schoolSet:schoolSet,transportSet:transportSet,drawSet:drawSet},
            success: function (data) {
...
$('#searchProperty').on('keyup keypress', function(e) {
...
//     var Mfooters = $('.inner-footers').height();
//     var MmainHeader = $('.main_header').height();
//     var MfilterForm = $('.list_search_area').height();
//     var Mtotalheight = Mfooters + MmainHeader + MfilterForm;
//     alert(Mtotalheight);
// })