...
// don't download content
	...
	public function feedData(){
		...
		$count=0;
		$allFeeds = $this->common_model->getAllwhere(SOURCE_FEEDS,array('id'=>60));
...
//$where = " name like '%".$prop['Unit_Type']."%'";
						...
						$dataInsert['publish_date'] = (isset($prop['Listing_Date']) && !empty($prop['Listing_Date']))?date('H:i:s Y-m-d',strtotime($prop['Listing_Date'])):'';
						$images = isset($prop['Images']['ImageUrl'])?$prop['Images']['ImageUrl']:'';
						$singleImg = array();
						$mainImg   = array();
						if(!empty($images)){
							/*to check if they are valid images*/
							...
							if(count($images)>1){
								...
								$singleImg[] = $this->uploadImageThumbnail($prop['Images']['ImageUrl']);
								$mainImg[] = $this->uploadImage($prop['Images']['ImageUrl']);
							}
							$dataInsert['thumbnail_photo_media'] = !empty($singleImg)?implode('|',$singleImg):'';
							$dataInsert['photo_media'] = !empty($mainImg)?implode('|',$mainImg):'';
						}
...
public function uploadImageThumbnail($url,$type=false){
	if($type && ($type == 'ycloud')){
		$name = date('ymdHis');
		$this->createThumbnail($url,350,200,FCPATH.'property_images/',$name.'.png');
		return base_url().'property_images/'.$name.'.png';
	}else{
...
public function uploadImage($url,$type=false){
	if($type && ($type == 'ycloud')){
		$name = date('ymdHis');
		file_put_contents(FCPATH.'property_images/'.$name.'.png', file_get_contents($url));
		return base_url().'property_images/'.$name.'.png';
	}else{
// $url_arr = parse_url($url);
// $query = $url_arr['query'];
