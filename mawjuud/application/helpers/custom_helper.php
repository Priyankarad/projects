...
if( ! function_exists('sendMail')) {
    function sendMail($to, $from, $message, $username,$subject,$property_share=false) {
        $CI = & get_instance();
        $config['mailtype'] = 'html';
        $CI->email->initialize($config);
        $mailData = array();
        ...
        if($property_share){
            ...
            $message = $CI->load->view('frontend/mailtemplate',$mailData,true);
        }
        $CI->email->from($from);
        $CI->email->to($to);
        $CI->email->subject($subject);
        $CI->email->message($message);
        $CI->email->set_header('From', $from);
        if($CI->email->send()){
            return true;
        }
        return false;
...
/*function for counting*/
if( ! function_exists('propertQuestions')) {
    function propertQuestions($propertyID) {
        $CI = & get_instance();
        $questionsData = $CI->common_model->getAllwhere(QUESTIONS,array('property_id'=>$propertyID));
