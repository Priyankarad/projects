...
$saveSearch;
if($this->session->userdata('save_search')){
    $saveSearch = $this->session->userdata('save_search');
}
?>
<?php
if($this->session->userdata('rentSale') && $this->session->userdata('rentSale')!=''){ ?>
    <input type="hidden" name="rentSale" id="rentSale" value="<?php echo $this->session->userdata('rentSale');?>">
<?php }else{ ?>
    <input type="hidden" name="rentSale" id="rentSale" value="0">
<?php }
?>

<?php if(isset($saveSearch['listing'])){ ?>
 <input type="hidden" name="rentSale1" id="rentSale1" value="<?php echo ($saveSearch['listing'] == 'sale')?'sale':'rent';?>">
<?php } ?>
<?php if(isset($saveSearch['listing'])){ ?>
 <input type="hidden" name="rentSale1" id="rentSale1" value="<?php echo ($saveSearch['listing'] == 'sale')?'sale':'rent';?>">
<?php } ?>
<?php if(isset($saveSearch['baths'])){ ?>
 <input type="hidden" name="baths1" id="baths1" value="<?php echo $saveSearch['baths'];?>">
<?php } ?>
<?php if(isset($saveSearch['beds'])){ ?>
 <input type="hidden" name="beds1" id="beds1" value="<?php echo $saveSearch['beds'];?>">
<?php } ?>
<?php if(isset($saveSearch['min_price'])){ ?>
 <input type="hidden" name="min1" id="min1" value="<?php echo str_replace(',','',$saveSearch['min_price']);?>">
<?php } ?>
<?php if(isset($saveSearch['max_price'])){ ?>
 <input type="hidden" name="max1" id="max1" value="<?php echo str_replace(',','',$saveSearch['max_price']);?>">
<?php } ?>
<section id="serch_prprty">
                ...
                <ul>
                    <li class="PsaveBtnS-lft2">
                        <div class="search_kew">
                            <input type="search" class="form-control" placeholder="Enter City & Area, Address, Neighborhood, Buildings" name="area" id="area" value="<?php 
                            if(isset($saveSearch['area'])){
                                echo $saveSearch['area'];
                            }else if(isset($searched_term)){
                                echo $searched_term;
                            }else{
                                echo '';
                            }
                    ...
                    <li class="sameSizeBli2">
                        ...
                        <select class="form-control myrslist" name="listing" id="listing" onchange="searchProperty();">
                            ...
                            if(isset($propertyType)){
                                if((isset($saveSearch['listing']) && ($saveSearch['listing'] == 'sale')) || $propertyType == 'sale' || ($this->session->userdata('rentSale') && $this->session->userdata('rentSale')=='sale') ){
                                    $sale_selected = 'selected=selected';
                                }if((isset($saveSearch['listing']) && ($saveSearch['listing'] == 'rent')) || $propertyType == 'rent' || ($this->session->userdata('rentSale') && $this->session->userdata('rentSale')=='rent')){
                                    $rent_selected = 'selected=selected';
                                }
                            }
                            ?>
                            <option value="all">Listing:</option>
                            <option value="sale" <?php echo $sale_selected;?>>Sale</optdion>
                            <option value="rent" <?php echo $rent_selected;?>>Rent</option>
                        </select>
                    </li>
                    <li class="sameSizeBli maxminp-mw">
                        <p><span class="appendPrice">Any Price</span> <svg class="caret" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M7 10l5 5 5-5z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg></p>
                        <div class="maxminp-submenu">
                            <div class="input-controls">
                                <input type="text" name="min_price" id="min_price" placeholder="Min" class="minvalues" onkeyup="priceChange();" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="<?php echo
                            isset($saveSearch['min_price'])?$saveSearch['min_price']:'';?>" />
                                <input type="text" name="max_price" id="max_price" placeholder="Max" class="maxvalues" onkeyup="priceChange();" oninput="this.value = this.value. replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="<?php echo
                            isset($saveSearch['max_price'])?$saveSearch['max_price']:'';?>"/>
                            </div>
                    ...
                    <li class="sameSizeBli">
                        <select class="form-control" id="beds" name="beds" onchange="searchProperty();">
                            <?php
                                $beds = isset($saveSearch['beds'])?$saveSearch['beds']:'';
                            ?>
                            <option value="all">Beds</option>
                            <option value="100" <?php ($beds == 100)?'selected':'';?>>Studio</option>
                            <?php
                                for($i=0;$i<7;$i++){ ?>
                                    <option value="<?php echo $i;?>" <?php ($beds == $i)?'selected':'';?>><?php echo $i;?>+</option>
                                <?php }
                            ?>
                        </select> 
                    </li>
                    <li class="sameSizeBli mw-typeselects">
                        <select class="form-control" multiple id="category" name="category[]" onchange="searchProperty();">
                            ...
                            if(!empty($categories['result'])){ 
                                ...
                                $category = isset($saveSearch['category'])?$saveSearch['category']:'';
                                if(!empty($category)){
                                    $checkedArr = $category;
                                }
                        ...
                        <div class="moremanyopt">
                            ...
                            <div class="MoreSmBox">
                                <div class="row">
                                    ...
                                    <div class="col s4">
                                        <?php
                                            $baths = isset($saveSearch['baths'])?$saveSearch['baths']:'';
                                        ?>
                                        <select class="form-control" name="baths" id="baths">
                                            <option value="all" <?php ($baths == 'all')?'selected':'';?> >Bath</option>
                                            <?php
                                            for($i=0;$i<6;$i++){ ?>
                                                <option value="<?php echo $i;?>" <?php ($baths == $i)?'selected':'';?>><?php echo $i;?>+</option>
                                            <?php }
                                            ?>
                                        </select> 
                                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col s4">
                                        <input type="text" id="min_sqft" name="min_sqft" placeholder="Min" value="<?php echo
                            isset($saveSearch['min_sqft'])?$saveSearch['min_sqft']:'';?>" />    
                                    </div>
                                    <div class="col s4">
                                        <input type="text" id="max_sqft" name="max_sqft" placeholder="Max" value="<?php echo
                            isset($saveSearch['max_sqft'])?$saveSearch['max_sqft']:'';?>"/>    
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col s8">
                                        <textarea placeholder="property,uae,..." id="keywords" name="keywords" rows="5"><?php echo
                            isset($saveSearch['keywords'])?$saveSearch['keywords']:'';?></textarea>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col s12">
                                        <button type="button" class="custombtnA waves-effect waves-light apply" onclick="searchProperty();"> Apply </button>   <button type="button" class="custombtnA clearbtndsn waves-effect waves-light"> Clear </button>
                                    </div>
                                </div>              
    ...
    <div id="titleModal" class="modal custompopupdesign">
        <a href="#!" class="modal-close waves-effect modal_closeA">&times;</a>
        <img class="logoTopFix" src="<?php echo site_url('assets/images/imglogopop.png') ?>" alt="img" />
        <p><center>Enter a Title</center></p>
        <div class="row">
            <div class="col s12">
                <input type="text" name="title" id="title" class="form-control mdl_eml" placeholder="Title">
                <input type="button" value="Submit" class="waves-effect waves-light titleSubmit">
            </div>
        </div>
    </div>
    <!--=============mini-search-bar============-->
    ...
    <div class="row">
        ...
        <div class="col s7 pd_rgtzero pd_lftzero M-listedfixedsec">
            ...
            <div class="srch_rsltlft scroll">
                ...
                <ul>
                    <li><a href="">About Mawjuud</a></li>
                    <li><a href="">Terms Of Use</a></li>
                    <li><a href="">Privacy Policy</a></li>
                    <li><a href="">Contact Us</a></li>
                </ul>
                <p>Copyright: © 2017-2018 Mawjuud. All rights reserved.</p>
            </div>
            </div>
...
<?php $this->session->set_userdata('save_search','');?>