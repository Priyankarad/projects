...
public function checkUser($data = array()){
    $this->db->select('id');
    $this->db->from(PROPERTY_USERS);
    $this->db->where(array('login_type'=>$data['login_type'],'oauth_uid'=>$data['oauth_uid']));
    $query = $this->db->get();
    $check = $query->num_rows();
    if($check > 0){
        $result = $query->row_array();
        $data['modified'] = date("Y-m-d H:i:s");
        $update = $this->db->update(PROPERTY_USERS,$data,array('id'=>$result['id']));
        $userID = $result['id'];
    }else{
        $data['created_date'] = date("Y-m-d H:i:s");
        $insert = $this->db->insert(PROPERTY_USERS,$data);
...
public function getThreeTableData($data,$table1,$table2,$table3,$on,$on2,$condition,$groupby,$order,$by){
    ...
    /*function for fetching the properties within the given miles*/
    public function getProperties($latitude,$longitude,$distance,$table,$where){
        $query = "property.*,( 3959 * acos( cos( radians(".$latitude.") ) * cos( radians( latitude ) ) 
